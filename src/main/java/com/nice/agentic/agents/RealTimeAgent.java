...
import com.nice.agentic.tools.StaffingSnapshotTool;
...
/**
 * RealTime Agent - specializes in LIVE data from Valkey.
 * <p>
 * Has access to: queue_state, realtime_staffing, metric_snapshot.
 */
@Component
public class RealTimeAgent implements SubAgent {

    private static final Logger log = LoggerFactory.getLogger(RealTimeAgent.class);
    ...
    public RealTimeAgent(BedrockRuntimeClient bedrock,
        ...
        this.systemPrompt = new String(
                new ClassPathResource("prompts/realtime-system.txt").getInputStream().readAllBytes(),
    ...
    public String investigate(String briefing, Map<String, Object> context) {
        log.info("RealTimeAgent investigating: {}", briefing);
        ...
        for (int iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
            ...
            if (response.stopReason() == StopReason.TOOL_USE && !toolUses.isEmpty()) {
                ...
                for (ToolUseBlock use : toolUses) {
                    ...
                    log.info("RealTimeAgent tool {} -> {} chars", use.name(), result.length());
            ...
            log.info("RealTimeAgent completed investigation: {} chars", finalText.length());
            return finalText;
        }

        log.warn("RealTimeAgent hit iteration limit");
        return "{\"findings\":[{\"metric\":\"unknown\",\"currentValue\":\"timeout\",\"observation\":\"Investigation did not converge\",\"evidence\":[]}]}";
    }
}
