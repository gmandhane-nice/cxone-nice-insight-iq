...
import com.nice.agentic.tools.AgentPerformanceTool;
...
/**
 * Context Agent - specializes in operational context (config changes, schedule changes, staffing changes).
 * <p>
 * Has access to: agent_timeline (via AgentPerformanceTool), ad_hoc_query.
 */
@Component
public class ContextAgent implements SubAgent {

    private static final Logger log = LoggerFactory.getLogger(ContextAgent.class);
    ...
    public ContextAgent(BedrockRuntimeClient bedrock,
        ...
        // AgentPerformanceTool provides agent timeline data
        List<AgentTool> tools = List.of(agentPerformanceTool, adHocQueryTool);
        ...
        this.systemPrompt = new String(
                new ClassPathResource("prompts/context-system.txt").getInputStream().readAllBytes(),
    ...
    public String investigate(String briefing, Map<String, Object> context) {
        log.info("ContextAgent investigating: {}", briefing);
        ...
        for (int iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
            ...
            if (response.stopReason() == StopReason.TOOL_USE && !toolUses.isEmpty()) {
                ...
                for (ToolUseBlock use : toolUses) {
                    ...
                    log.info("ContextAgent tool {} -> {} chars", use.name(), result.length());
            ...
            log.info("ContextAgent completed investigation: {} chars", finalText.length());
            return finalText;
        }

        log.warn("ContextAgent hit iteration limit");
        return "{\"findings\":[{\"eventType\":\"unknown\",\"timestamp\":\"unknown\",\"observation\":\"Investigation did not converge\",\"evidence\":[]}]}";
    }
}
